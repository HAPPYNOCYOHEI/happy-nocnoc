 ...
 *  @description 官方新增组件：用于首页/内容页展示「背景图 + 文案 + 产品卡片」
 */
if (!customElements.get('featured-product-card')) {
  class FeaturedProductCard extends HTMLElement {
    ...
    connectedCallback() {
      // NOTE: SplitText 不是所有站点都会加载；缺失时直接跳过动画，避免报错
      if (document.body.classList.contains('animations-true') && typeof gsap !== 'undefined' && typeof SplitText !== 'undefined') {
        this.prepareAnimations();
      }
    }
    disconnectedCallback() {
      if (document.body.classList.contains('animations-true') && typeof gsap !== 'undefined' && this.tl) {
        this.tl.kill();
